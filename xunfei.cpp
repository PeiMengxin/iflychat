...
//	using namespace Microsoft::DirectX::DirectSound;
	...
	public: int status()
			...
			int TextToSpeed(CString Ssrc_text)//字符串转音频，音频存放在PCM_SPEED.pcm下
			{
			...
			CString GetPcmName()//获取音频文件路径
			{
				return gcnew CString(pcm_path.c_str());
			}
			
			int Play(CString text)//播放音频文件
			{
				if(text == "") return -1;
